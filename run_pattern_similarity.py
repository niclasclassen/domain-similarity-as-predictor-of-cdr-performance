from utils import get_all_domains, get_domain_reviews, count_overlapping_ids


def get_rating_pattern_similarities(domain):
    domain_statistics = {}
    rating_embeddings_by_user = {}
    rating_embeddings_by_item = {}

    domains = get_all_domains()
    # for domain in domains:
    print(f"Processing domain: {domain}")
    raw_reviews = get_domain_reviews(domain)

    # calculate basic dataset statistics
    # domain_statistics[domain] = {
    #     "num_reviews": len(raw_reviews),
    #     "num_users": len(raw_reviews["user_id"].unique()),
    #     "num_items": len(raw_reviews["parent_asin"].unique()),
    #     "user_ids": raw_reviews["user_id"].unique(),
    #     "item_ids": raw_reviews["parent_asin"].unique(),
    # }

    # # reviews_df = process_reviews(raw_reviews)

    # unique_user_ids = reviews_df["user_id"].unique()
    # unique_item_ids = reviews_df["parent_asin"].unique()
