...
class Hit(TopkMetric):
    ...
    def calculate_metric(self, dataobject):
        ...
        # Calculate hits for each user (1 if at least one hit in top-k, 0 otherwise)
        user_hits = np.any(result[:, :10], axis=1).astype(int)

        # Write the user hits to a file
        with open("user_hits.txt", "w") as f:
...
class MRR(TopkMetric):
    ...
    def calculate_metric(self, dataobject):
        ...
        # Calculate the MRR@10 for each user
        user_mrrs = self.calculate_mrr_at_10(result)

        # Write the user MRRs to a file
        with open("user_mrrs.txt", "w") as f:
            for user_mrr in user_mrrs.values():
                f.write(f"{user_mrr}\n")
    ...
    def calculate_mrr_at_10(self, result):
        user_mrrs = {}
        for user_idx, user_result in enumerate(result):
            top_10 = user_result[:10]
            mrr = 0.0
            for rank, score in enumerate(top_10, start=1):
                if score > 0:
                    mrr = 1.0 / rank
                    break
            user_mrrs[user_idx] = mrr
        return user_mrrs
...
class NDCG(TopkMetric):
    ...
    def calculate_metric(self, dataobject):
        ...
        # Calculate the NDCG@10 for each user
        user_ndcg_at_10 = self.calculate_ndcg_at_10(result)

        # write the user NDCG@10 to a file
        with open("user_ndcg_at_10.txt", "w") as f:
            for user_ndcg in user_ndcg_at_10.values():
                f.write(f"{user_ndcg}\n")
    ...
    def calculate_ndcg_at_10(self, result):
        user_ndcg_at_10 = {}
        for user_idx, user_result in enumerate(result):
            ndcg_at_10 = user_result[9]  # NDCG@10 is the value at the 10th position
            user_ndcg_at_10[user_idx] = ndcg_at_10
        return user_ndcg_at_10
